...
class EditProfilePic extends StatefulWidget {
  final userid;
  EditProfilePic ({ Key? key, required this.userid}): super(key: key);
  @override
  _EditProfilePic createState() => _EditProfilePic();
}
...
class _EditProfilePic extends State<EditProfilePic> {
  ...
  save(image) async{
    ...
    if(image != null){
      print("path is");
      print(image.path);
      var ref = firebase_storage.FirebaseStorage.instance.ref('profileimages/' + widget.userid.toString() + '.png');
      UploadTask task = ref.putFile(image);
      url = await (await task).ref.getDownloadURL();
      FirebaseFirestore.instance.collection('users').doc(widget.userid).update({"image_path": url});
    }
    Navigator.push(
        context,
        MaterialPageRoute(
